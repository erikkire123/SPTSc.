...
--vars
	...
	task.spawn(function()
		function arespawn()
while true do wait()
	if game:GetService("Players").LocalPlayer.PlayerGui.Play.Enabled == true then
		Event:FireServer()
	...
	humanoid.Died:Connect(arespawn)
				...
				while wait() do
					if _G.updb == true  then
						...
						for i,v in pairs(Zones.BT:GetChildren()) do
							if v:IsA("BasePart") then
								if BT >= v.Damage.Value * 2 then
									table.insert(availableBT, v.Damage.Value * 2)
									wait()
								end
							end
						end
