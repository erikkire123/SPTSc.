...
--vars
	...
	warn("You cant put something other than a number or false")
		...
		local Event = game:GetService("ReplicatedStorage").RespawnEvent
			if plr.PlayerGui.Play.Enabled == true then
			Event:FireServer()
			plr.PlayerGui.Play.Enabled = false
			wait(5.5)
		end
	...
	task.spawn(function()
 while wait() do
  				if  game:GetService("Players").LocalPlayer.PlayerGui.Play.Enabled == true and game.Players.LocalPlayer.Character.Humanoid.Health == 0 and getgenv().Config.ServerHopOnDeath == false then
local Event = game:GetService("ReplicatedStorage").RespawnEvent
Event:FireServer()

game:GetService("Players").LocalPlayer.PlayerGui.plrstats.Enabled = true
game:GetService("Players").LocalPlayer.PlayerGui.Play.Enabled = false
